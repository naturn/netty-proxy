package com.lyzh.netty.gateway.common;
...
public final class Conversion {
    ...
    public static String getCrc(String hexStr) {
        ...
        int pData = 0;
        while (wDataLen > 0) {
            fcs = (fcs >> 8) ^ crctab16[(fcs ^ data[pData]) & 0xff];
            wDataLen--;
            pData++;
        }
        fcs = fcs ^ 0xffff;
        return Integer.toHexString(fcs).toUpperCase();
    }
    ...
    public static String bytesToHexString(Iterator<Byte> bytes) {
    ...
    public static int bytesToIntForLength(byte[] src, int offset) {
        int value;
        value = (int) ((src[offset + 1] & 0xFF) << 8 | ((src[offset] & 0xFF)) | ((src[offset + 3] & 0xFF) << 16)
    ...
    public static int hexStringToInt(String src) {
        int value;
        value = ((Integer.parseInt(src.charAt(0) + "")) << 12) + (Integer.parseInt(src.charAt(1) + "") << 8)
                + (Integer.parseInt(src.charAt(2) + "") << 4) + (Integer.parseInt(src.charAt(3) + "") << 0);
        return value;
    }

    public static int bytesToIntForFiveFive(byte[] src) {
        int value;
        value = Integer.parseInt(String.valueOf((char)src[0])) << 4; 
                value += Integer.parseInt(String.valueOf((char)src[1])) << 0; 
                value += Integer.parseInt(String.valueOf((char)src[2])) << 12;
                value += Integer.parseInt(String.valueOf((char)src[3])) << 8;
        return value;
    }
